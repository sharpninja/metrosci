...
PRectangle Editor::GetTextViewRectangle() {
	return GetTextRectangle();
}

int Editor::LinesOnScreen() {
	PRectangle rcClient = GetTextViewRectangle();
...
Point Editor::LocationFromPosition(SelectionPosition pos) {
	...
	if (surface && ll) {
		...
		pt.y = (lineVisible - GetTopLineGraphics() - 1) * vs.lineHeight;
		pt.x = 0;
...
int Editor::LineFromLocation(Point pt) {
	return cs.DocFromDisplay(pt.y / vs.lineHeight + GetTopLineGraphics());
}
...
SelectionPosition Editor::SPositionFromLocation(Point pt, bool canReturnInvalid, bool charPosition, bool virtualSpace) {
	...
	int visibleLine = pt.y / vs.lineHeight + GetTopLineGraphics();
	if (pt.y < 0) {	// Division rounds towards 0
		visibleLine = (static_cast<int>(pt.y) - (vs.lineHeight - 1)) / vs.lineHeight + GetTopLineGraphics();
	}
...
PRectangle Editor::RectangleFromRange(int start, int end) {
	...
	PRectangle rc;
	rc.left = vs.fixedColumnWidth;
	rc.top = (minLine - GetTopLineGraphics()) * vs.lineHeight;
	if (rc.top < 0)
		rc.top = 0;
	rc.right = rcClient.right;
	rc.bottom = (maxLine - GetTopLineGraphics() + 1) * vs.lineHeight;
...
void Editor::MoveCaretInsideView(bool ensureVisible) {
	PRectangle rcClientView = GetTextViewRectangle();
	Point pt = PointMainCaret();
	if (pt.y < rcClientView.top) {
		MovePositionTo(SPositionFromLocation(
		            Point(lastXChosen - xOffset, rcClientView.top),
					false, false, UserVirtualSpace()),
					Selection::noSel, ensureVisible);
	} else if ((pt.y + vs.lineHeight - 1) > rcClientView.bottom) {
		int yOfLastLineFullyDisplayed = rcClientView.top + (LinesOnScreen() - 1) * vs.lineHeight;
		MovePositionTo(SPositionFromLocation(
		            Point(lastXChosen - xOffset, rcClientView.top + yOfLastLineFullyDisplayed),
					false, false, UserVirtualSpace()),
		        Selection::noSel, ensureVisible);
	}
}
...
void Editor::PaintSelMargin(Surface *surfWindow, PRectangle &rc) {
	...
	for (int margin = 0; margin < vs.margins; margin++) {
		if (vs.ms[margin].width > 0) {
			...
			int visibleLine = GetTopLineGraphics() + lineStartPaint;
			...
			if (vs.ms[margin].mask & SC_MASK_FOLDERS) {
				...
				if (highlightDelimiter.isEnabled) {
					int lastLine = cs.DocFromDisplay(GetTopLineGraphics() + LinesOnScreen()) + 1;
...
void Editor::Paint(Surface *surfaceWindow, PRectangle rcArea) {
	...
	PRectangle rcClient = GetClientRectangle();
	rcClient.top = rcArea.top;
	rcClient.bottom = rcArea.bottom;
	...
	if (rcArea.right > vs.fixedColumnWidth) {
		...
		int visibleLine = GetTopLineGraphics() + screenLinePaintFirst;
		...
		rcBeyondEOF.top = (cs.LinesDisplayed() - GetTopLineGraphics()) * vs.lineHeight;
...
int Editor::PositionAfterArea(PRectangle rcArea) {
	...
	int lineAfter = GetTopLineGraphics() + (rcArea.bottom - 1) / vs.lineHeight + 1;

...
// Windows RT has its own Platform namespace which interferes with a class called Platform

namespace Platform {

Scintilla::ColourDesired Chrome();
Scintilla::ColourDesired ChromeHighlight();
...
long SendScintilla(
	Scintilla::WindowID w, unsigned int msg, unsigned long wParam=0, long lParam=0);
long SendScintillaPointer(
	Scintilla::WindowID w, unsigned int msg, unsigned long wParam=0, void *lParam=0);
bool IsDBCSLeadByte(int codePage, char ch);
int DBCSCharLength(int codePage, const char *s);
int DBCSCharMaxLength();
